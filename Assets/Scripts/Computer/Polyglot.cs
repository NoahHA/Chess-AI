...
/// <summary>
/// Class for decoding polyglot opening books, format taken from http://hgm.nubati.net/book_format.html.
/// </summary>
public static class Polyglot
{
    private static byte[] bookBytes;

    // Array of the concatenation of randomPiece, randomCastle, randomEnPassant, and randomTurn
    private static readonly ulong[] random64 = new ulong[] {
   ...
   0xF8D626AAAF278509,
    ...
    private static readonly Dictionary<char, int> pieceTypes = new Dictionary<char, int>()
    {
        { 'p', 0},
        { 'P', 1 },
        { 'n', 2 },
    ...
    private static readonly Dictionary<char, int> castleTypes = new Dictionary<char, int>()
    {
        { 'K', 0 },
        { 'Q', 1 },
        { 'k', 2 },
    ...
    public static ulong ConvertBoardToKey(Board board)
    {
        ulong[] randomPiece = random64[0..768];
        ulong[] randomCastle = random64[768..772];
        ulong[] randomEnPassant = random64[772..780];
        ulong randomTurn = random64.Last();

        List<ulong> pieceValues = new();
        List<ulong> castleValues = new();

        for (int i = 0; i < 64; i++)
        {
            Square square = new(i);
            Piece piece = board.State[i];

            if (piece is not null)
            {
                int pieceOffset = 64 * pieceTypes[piece.Letter] + 8 * (square.Row - 1) + (square.Col - 1);
                pieceValues.Add(randomPiece[pieceOffset]);
            }
        }

        foreach (char castleOption in board.FEN.GetFenCastleSection())
        {
            castleValues.Add((castleOption == '-') ? 0 : randomCastle[castleTypes[castleOption]]);
        }

        ulong pieceValue = pieceValues.Aggregate((current, next) => (current ^ next));
        ulong castleValue = castleValues.Aggregate((current, next) => (current ^ next));
        ulong enPassantValue = (board.EnPassantSquare is null || !board.CanTakeEnPassant()) ? 0 : randomEnPassant[board.EnPassantSquare.Col - 1];
        ulong turnValue = (board.Turn == PieceColour.White) ? randomTurn : 0;

        return pieceValue ^ castleValue ^ enPassantValue ^ turnValue;
    }

    public static void ReadOpeningsBook(string filePath)
    {
        bookBytes = File.ReadAllBytes(filePath);
    }

    private static List<Move> GetAllMoves(Board board)
    {
        const int EntrySize = 16;
        int entryIndex = 0;
        ulong boardKey = ConvertBoardToKey(board);
        List<Move> moves = new();

        while (entryIndex + EntrySize <= bookBytes.Length)
        {
            ulong key = BitConverter.ToUInt64(bookBytes, entryIndex);

            if (boardKey == key)
            {
                moves.Add(ConvertBytesToMove(BitConverter.ToUInt16(bookBytes, entryIndex + 8)));
            }
    ...
    {
        // TODO:
        // Weigh move selection probability by count (the next 16 bits in the entry after move)

        List<Move> allMoves = GetAllMoves(board);
        System.Random random = new System.Random();
        Move randomMove = allMoves[random.Next(0, allMoves.Count)];

        // White Kingside castling
        if (randomMove.ToString() == "e1h1" && board.CanCastle[0])
        {
            randomMove = new Move("e1", "g1", castling: true);
        }

        // White Queenside castling
        else if (randomMove.ToString() == "e1a1" && board.CanCastle[1])
        {
            randomMove = new Move("e1", "c1", castling: true);
        }

        // Black Kingside castling
        else if (randomMove.ToString() == "e8h8" && board.CanCastle[2])
        {
            randomMove = new Move("e8", "g8", castling: true);
        }

        // Black Queenside castling
        else if (randomMove.ToString() == "e8a8" && board.CanCastle[3])
        {
            randomMove = new Move("e8", "c8", castling: true);
        }
    ...
    private static Move ConvertBytesToMove(ushort move)
    {
        // TODO:
        // Implement promotions

        ushort toCol = (ushort)(move & 0x0007); // Bits 0 to 2
        ushort toRow = (ushort)((move >> 3) & 0x0007); // Bits 3 to 5
        ushort fromCol = (ushort)((move >> 6) & 0x0007); // Bits 6 to 8
        ushort fromRow = (ushort)((move >> 9) & 0x0007); // Bits 9 to 11
        ushort promotionPiece = (ushort)((move >> 12) & 0x0007); // Bits 12 to 14
        ...
        return new Move(new Square(fromCol, fromRow), new Square(toCol, toRow));
    }
}